<?php include "partials/header.php" ?>
<?php include "partials/nav.php" ?>
		<!-- welcome-hero end  -->

        <!-- body of the page start  -->
		<section class="shop-body">
			<!-- welcome to shop page  -->
			<div class="welcome-shop-container">
				<div class="welcome-title">
					<div class="welcome-intro">
						<p>Great Design Collection</p>
						<h1>Cloth Covered Accent Chair</h1>
						<p>Lorem Ipsum Dolor Sit Amet, Consectetur Adipisicing Elit, Sed Do Eiuiana Smod Tempor Ut Labore Et Dolore Magna Aliqua. Ut Enim Ad Minim Veniam, Quis Nostrud Exercitation Ullamco Laboris Nisi Ut Aliquip.</p>
						<div class="welcome-pricing">
							<div class="line-before"></div>
							<p class="new-price">$ 399.00 </p>
							<p class="old-price">$ 499.00 </p>
						</div>
						<div class="welcome-shop-buttons">
							<button class="btn-add btn-contact-us">Add to Cart</button>
							<button class="btn-more">More Info</button>
						</div>
					</div>
					<div class="welcome-img">
						<img src="./assets/images/slider/slider1.png" alt="">
					</div>
				</div>
			</div>
			<!-- welcome page end  -->
			<!-- populer products  start -->
			<div class="populer-products-container">
				<div class="cards-containr">
					<div class="card">
						<div class="card-img">
							<img src="./assets/images/populer-products/p1.png" alt="">
						</div>
						<h3>Arm Chair</h3>
						<p>Nemo enim ipsam voluptatem quia volu ptas sit asperna aut odit aut fugit.</p>
					</div>
					<div class="card card-discover-more">
						<div class="card-img">
							<img src="./assets/images/populer-products/p2.png" alt="">
						</div>
						<div class="card-content">
							<h4>Latest Designed Stool and Chair</h4>
							<p>Edi Ut Perspiciatis Unde Omnis Iste Natusina Error Sit Voluptatem Accusantium Doloret Mque Laudantium, Totam Rem Aperiam.</p>
							<h5>Sales Start from <span>$99.00</span></h5>
							<button class="s-btn-dis-more">Discover more </button>
						</div>
					</div>
					<div class="card">
						<div class="card-img">
							<img src="./assets/images/populer-products/p3.png" alt="">
						</div>
						<h3>Hanging Lamp</h3>
						<p>Sit Voluptatem Accusantium Doloret Mque Laudantium, Totam Rem Aperiam.</p>
					</div>
				</div>
			</div>
			<!-- populer products  end -->
			<!-- All products container start  -->
			<div class="all-products-container">
				<div class="all-products-title">
					<div class="filter-options">
						<p class="active">All products</p>
						<p class="">New</p>
						<p class="">Best</p>
						<p class="">price</p>
						<p class="">release date</p>
					</div>
					<button class="btn-filter btn-see-all">
						Filter
					</button>
				</div>
				<div class="all-pr-container">
					<!-- new products section  start -->
					<div class="new-products-section">
						<div class="new-products-body">
							<div class="new-products-body-card-discount">
								<div class="discount-circle">
									<p> -50% </p>
								</div>
								<h3>House Chair Sale <span class="summer-word">Summer</span> Store </h3>
							</div>
							<div class="new-product-card">
								<div class="new-pr-crd-img">
									<img src="./assets/images/collection/arrivals2.png" alt="">
								</div>
								<p>Latest Designed Stool and Chair</p>
								<p>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
								</p>
								<div class="new-product-card-buy-price">
									<p>100$</p>
									<div>
										<img src="./assets/images/icons/icon-shop.svg" alt="">
									</div>
								</div>
							</div>
							<div class="new-product-card">
								<div class="new-pr-crd-img">
									<img src="./assets/images/collection/arrivals3.png" alt="">
								</div>
								<p>Latest Designed Stool and Chair</p>
								<p>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
								</p>
								<div class="new-product-card-buy-price">
									<p>100$</p>
									<div>
										<img src="./assets/images/icons/icon-shop.svg" alt="">
									</div>
								</div>
							</div>
							<div class="new-product-card">
								<div class="new-pr-crd-img">
									<img src="./assets/images/collection/arrivals1.png" alt="">
								</div>						<p>Latest Designed Stool and Chair</p>
								<p>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
								</p>
								<div class="new-product-card-buy-price">
									<p>100$</p>
									<div>
										<img src="./assets/images/icons/icon-shop.svg" alt="">
									</div>
								</div>
							</div>
							<div class="new-product-card">
								<div class="new-pr-crd-img">
									<img src="./assets/images/collection/arrivals4.png" alt="">
								</div>						<p>Latest Designed Stool and Chair</p>
								<p>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<i class="fa fa-star"></i>
									<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
								</p>
								<div class="new-product-card-buy-price">
									<p>100$</p>
									<div>
										<img src="./assets/images/icons/icon-shop.svg" alt="">
									</div>
								</div>
							</div>
						</div>
					</div>
					<!-- new products section end -->
				</div>
			</div>
			<!-- All products container end  -->
			<!-- Discount cover start  -->
			<div class="discount-cover-container">
				<h1>Discount up to </h1>
				<div class="discount-circle">
					<p> -50% </p>
				</div>
				<h1>this season</h1>
			</div>
			<!-- Discount cover end  -->
			<!-- .best-selling products start  -->
			<div class="best-selling-products-section">
				<div class="best-selling-products-title ">
					<div class="new-pr-title">
						<h3>Best-selling Products</h3>
						<div class="orange-line"></div>
					</div>
					<div class="see-all-products-btn">
						<button class="btn-see-all">See All</button>
					</div>
				</div>
				<div class="best-selling-cards">

				
				<div class="best-selling-products-card">
					<div class="best-selling-products-card-img">
						<img src="./assets/images/collection/arrivals6.png" alt="">
					</div>
					<div class="best-selling-products-card-review">
						<div class="best-selling-products-card-review-stars">
							<p>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
							</p>

						</div>
						<div class="best-selling-products-card-review-count">
							<p>(45 Review )</p>
						</div>
						
					</div>
					<p>Designed Sofa</p>
					<p>$160</p>
				</div>
				<div class="best-selling-products-card">
					<div class="best-selling-products-card-img">
						<img src="./assets/images/collection/arrivals4.png" alt="">
					</div>
					<div class="best-selling-products-card-review">
						<div class="best-selling-products-card-review-stars">
							<p>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
							</p>

						</div>
						<div class="best-selling-products-card-review-count">
							<p>(45 Review )</p>
						</div>
						
					</div>
					<p>Designed Sofa</p>
					<p>$160</p>
				</div>
				<div class="best-selling-products-card">
					<div class="best-selling-products-card-img">
						<img src="./assets/images/collection/arrivals7.png" alt="">
					</div>
					<div class="best-selling-products-card-review">
						<div class="best-selling-products-card-review-stars">
							<p>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
							</p>

						</div>
						<div class="best-selling-products-card-review-count">
							<p>(45 Review )</p>
						</div>
						
					</div>
					<p>Designed Sofa</p>
					<p>$160</p>
				</div>
				<div class="best-selling-products-card">
					<div class="best-selling-products-card-img">
						<img src="./assets/images/collection/arrivals3.png" alt="">
					</div>
					<div class="best-selling-products-card-review">
						<div class="best-selling-products-card-review-stars">
							<p>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<i class="fa fa-star"></i>
								<span class="spacial-feature-icon"><i class="fa fa-star"></i></span>
							</p>

						</div>
						<div class="best-selling-products-card-review-count">
							<p>(45 Review )</p>
						</div>
						
					</div>
					<p>Designed Sofa</p>
					<p>$160</p>
				</div>
				</div>
            </div>
			<!-- .best-selling products end -->
			<!-- footer of the page start  -->
             <!-- footer start  -->
            
<?php require "partials/footer.php"?>